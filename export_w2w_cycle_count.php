...
redirect("login", false);
    ...
    print_r_html('Export Page');

    $db_inventory_details = $db->query('SELECT * FROM tb_inventory_adjustment WHERE qty_case-allocated_qty <> ? and transaction_type = ? AND putaway_status = ?',0,'INB','Done')->fetch_all();

    $db_inv = $db->query('SELECT
    a.id,
    a.ia_ref,
    a.actual_bin_loc,')

    // print_r_html($db_inventory_details);
