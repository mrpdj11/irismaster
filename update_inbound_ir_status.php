...
if (isset($_POST)) {
  ...
  if (are_fields_filled($_POST)) {
    ...
    $update_inbound = $db->query('UPDATE tb_inbound set status=?  WHERE document_no = ? ', '1', $_POST['document_no']);
    if ($update_ir_status->affected_rows() && $update_inbound->affected_rows()) {
