...
<?php
$today = date('Y-m-d');
$outbound_report = $db->query('SELECT a.ref_no,a.document_no,a.item_code,a.batch_no,a.qty_pcs,a.lpn,a.date_time,a.fullfilled_by,b.ship_date,c.branch_name,d.material_description,e.status,b.document_name
                    ...
                    WHERE b.document_name= ?', $_GET['document_no'])->fetch_all();

//print_r_html($outbound_report);
foreach ($outbound_report as $arr_key => $arr_val) {
  $doc = $arr_val['document_no'];
  $source = $arr_val['branch_name'];
...
$tb_header->easyCell('OUTBOUND FULLFILLMENT REPORT', 'font-size:18; font-style:B; align:L; valign:M');
...
$table_item_list = new easyTable($pdf, '{35,35,150,60}');
...
$i = 1;
foreach ($outbound_report as $db_in_key => $db_in_val) {
  $bgcolor = '';
  if ($i % 2) {
...
$pdf->Output('', "OUTBOUND FULLFILLMENT REPORT-" . $_GET['document_no'] .  ".pdf"); //To Print and to indicate the filename
